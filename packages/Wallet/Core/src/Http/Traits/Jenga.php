...
trait Jenga
{
    ...
    private function jengaFetchToken($account)
    {
        $token = $this->jengaGenerateToken($account);
        if (!$token) {
            info("JENGA TOKEN ERROR: " . json_encode($token));
            return false;
        }
        info('jengaFetchToken: ' . json_encode($token));
        return $token->accessToken;
    }

    private function jengaGenerateToken($account)
    {
    ...
    private function jengaGenerateSignature($signaturePlainText)
    {
    ...
    private function jengaFetchBalance($account)
    {
        ...
        return $this->jengaCurlGet($signaturePlainText, $account, $url);
    }

    private function jengaGetMiniStatement($account)
    {
    ...
    private function jengaGetFullStatement($account, $start_date, $end_date, $limit = 3)
    {
        ...
        return $this->jengaCurlPost($signaturePlainText, $account, $url, $data);
    }

    private function jengaSendToEquity($account, $destination)
    {
    ...
    private function jengaSendToMobile($account, $destination)
    {
        $destination = (object) $destination;
        info("----------------------------- SEND TO MPESA ---------------------------");
    ...
    private function jengaSendRtgs($account, $destination)
    {
    ...
    private function jengaSendSwift($account, $destination)
    {
        //transfer.reference transfer.date source.accountNumber destination.accountNumber transfer.amount
        info("----------------------------- SWIFT ---------------------------");
        $destination = (object) $destination;
    ...
    private function jengaPesalinkBank($account, $destination)
    {
    ...
    private function jengaPesalinkMobile($account, $destination)
    {
        //transfer.amount transfer.currencyCode transfer.reference destination.name source.accountNumber
    ...
    private function jengaCurlGet($signaturePlainText, $account, $url)
    {
        $client = new Client();
        try {
            $response = $client->request('GET', $url, [
                'headers' => [
                    'Authorization' => 'Bearer ' . $this->jengaFetchToken($account),
                    'signature' => $this->jengaGenerateSignature($signaturePlainText),
                ],
            ]);
    ...
    private function jengaCurlPost($signaturePlainText, $account, $url, $data)
    {
        $client = new Client();
